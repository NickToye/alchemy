  ...
  render() {
    return (
      <div className="u-margin-bottom-huge">
        <h2 className="u-alchemy-primary-colour">Architectural Principles</h2>
        <p>
          To many developers CSS is seen as a loose language, and not really worthy of architecture - but the
          very fact that it is loose supports the need to have it well designed from the start.
        </p>
        <p>
...
.c-btn--soft {
  border-radius: 6px;
        ...
        <p>
          So the <code className="u-alchemy-positive-colour">.btn {}</code> class provides structural styling
          to the element, whilst <code className="u-alchemy-positive-colour">.c-btn--positive</code> and{' '}
          <code className="u-alchemy-positive-colour">.c-btn--soft</code> provides cosmetic styling.
        </p>
        <small className="a-code-label u-alchemy-positive-colour">
...
.box--large {
  padding: 20px;
        ...
        <ul className="u-margin-bottom-large">
          ...
          <li>
            the <code className="u-alchemy-positive-colour">.box</code> is now unpredictable to developers and
            the Single Responsibility no longer exists because nesting the selector produces a forced caveat.
          </li>
        </ul>
        <p>
          When working in a team environment, be sure to write API-like CSS; always ensure that existing
          classes remain backward compatible (i.e. no changes at their root) and provide new hooks to bring in
          new features. Changing the root object, abstraction, or component could have huge knock-on effects
          for developers making use of that code elsewhere, so never modify existing code directly.
        </p>
        <h3>DRY</h3>
...
`}</code>
        ...
        <p>
          Now the two declarations only exist once, and if we ever needed to change the typeface or weight, we
          need only do it in one place.
        </p>
        ...
        <p>
          ...
          rather than inheriting behaviour from larger objects. This will keep your code decoupled as nothing
          inherently depends on anything else. Another example of this is to think of it like Atomic Design.
        </p>
        <p>
          Atomic Design principles are you start with the smallest <strong>atomic</strong> elements, build up
          to <strong>molecules</strong>, and combine to <strong>organisms</strong>, and so on.
        </p>
        ...
        <blockquote>
          <p>
            into distinxt sections, such that each section addresses a separate concern. A concern is a set of
            information that affects the code of a computer program. A program that embodies SoC well is
            called modular.
          </p>
        </blockquote>
        <p>
          In terms of CSS, this just means we write code that only ever focusses itself on one task at a time.
          This way you can be sure that you are not going to break other semantically unrelated pieces of UI.
          This reduces the risk of failure, and should reduce the amount of bugs found in your system.
        </p>
        <p>
