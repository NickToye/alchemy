  ...
  render() {
    return (
      <div className="u-margin-bottom-huge">
        <h2 className="u-alchemy-primary-colour">Naming Conventions</h2>
        <p>
        ...
        <div className="a-note u-alchemy-caution-bg u-padding-small u-margin-bottom">
          <small className="u-alchemy-white-colour">
            <FontAwesomeIcon icon="exclamation-triangle" />&nbsp; However, please try and not rely on BEM too
            heavily, consider whether a simple hyphenated class would suffice.
          </small>
        </div>
...
.footer { }
  ...
  .page__footer { }
    ...
    .page__copyright { }`}</code>
        ...
        <p>
          On this occasion we would use a descendant selector to modify an <strong>Element</strong> based on a{' '}
          <strong>Modifier</strong> on the <strong>Block</strong>.
        </p>
...
.box--red { }`}</code>
        ...
        <p>
          ...
          <code className="u-alchemy-positive-colour">.box--red {}</code> instead, we make use of Sass’ parent
          selectors to prepend <code className="u-alchemy-positive-colour">.box--red</code> onto the existing{' '}
          <code className="u-alchemy-positive-colour">.box__ball {}</code> selector. This means that all of
          our <code className="u-alchemy-positive-colour">.box__ball {}</code> -related rules exist in once
          place, and aren’t spread throughout the file. This is general good practice when dealing with nested
          code: keep all of your context (e.g. all{' '}
          <code className="u-alchemy-positive-colour">.box__ball {}</code> code) encapsulated in one location.
        </p>
...
`}</code>
  ...
  `}</code>
        ...
        <p>
          Now we can see which classes are related to each other, and how. We know what classes we can&apos;t
          use outside of the scope of this piece of UI, and we know which classes we are free to reuse
          elsewhere.
        </p>
        ...
        <p>
          ...
          team understand that know JS functionality will be utilising and CSS classes then you are generally
          free to working within the CSS without fear of breaking the JavaScript.
        </p>
        <p>
        ...
        <p>
          ...
          <code className="u-alchemy-positive-colour">data-*</code> attributes are used to <em>store data</em>,
          and not be <em>bound to</em>.
        </p>
      </div>
    );
  }
